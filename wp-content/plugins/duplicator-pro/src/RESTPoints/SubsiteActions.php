...
class SubsiteActions extends \Duplicator\Core\REST\AbstractRESTPoint
{
            ...
            'data' => array(
                ...
                'validate_callback' => function ($param, \WP_REST_Request $request, $key) {
                    ...
                    foreach ($param as $item) {
                        ...
                        if ($item['targetId'] == 0 && strlen('newSlug') == 0) {
                            return false;
                        }
                        if ($item['targetId'] > 0 && get_site($item['targetId']) == null) {
                            return false;
                        }
                    }
                    return true;
    ...
    protected function respond(\WP_REST_Request $request, $responseBase)
    {
        ...
        foreach ($data as $subsiteInfo) {
            if ($subsiteInfo['targetId'] > 0) {
                ...
                $newBlogId = self::createNewSubsite($subsiteInfo['newSlug'], $subsiteInfo['blogName'], $currentUserId);
     ...
     */
    protected static function createNewSubsite($newSlug, $blogTitle, $adminUserId)
    {
