...
class AffiliateWPFF extends Affiliate_WP_Base
{
    ...
    public function boot()
    {
        ...
        /* Regular Payments */
        add_action('fluentform_submission_inserted', [$this, 'addPendingReferral'], 99, 3);
        add_action('fluentform_process_payment', [$this, 'onProcesPayment'], 99, 6);
        add_action('fluentform_after_transaction_status_change', [$this, 'markReferralComplete'], 10, 3);
        add_action('fluentform_payment_refunded', [$this, 'revokeReferralOnRefund'], 10, 3);

        /* Subscription Payments */
        add_action('fluentform_subscription_payment_canceled', [$this, 'revokeReferralOnRefund'], 10, 3);
        add_action('fluentform_subscription_payment_received', function ($subscription, $submission) {
    ...
    public function addPendingReferral($insertId, $formData, $form, $totalPayable = 0)
    {
        ...
        if (empty($amountInCents) || $submission->payment_status == 'paid') {
            ...
            do_action('ff_log_data', [
                ...
                'description'      => sprintf(
                    __('Referral #%1$d for %2$s recorded for %3$s (ID: %4$d).', 'fluentform'),
    ...
    public function revokeReferralOnRefund($refund, $transaction, $submission)
    {
        ...
        $name = affiliate_wp()->affiliates->get_affiliate_name($referral->affiliate_id);
        $note = sprintf(
            __('Referral #%d for %s for %s rejected', 'affiliate-wp'),
            $referral->referral_id,
            $amount,
            $name
        );
    ...
    public function referenceLink($reference = 0, $referral)
    {
    ...
    {
        add_action('fluentform_after_save_form_settings', function ($formId, $allSettings) {
