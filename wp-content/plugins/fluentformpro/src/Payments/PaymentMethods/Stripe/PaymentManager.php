...
class PaymentManager
{
    public function cancelSubscription($subscription, $scope = 'admin', $submission = false)
    {
        ...
        if(!$submission || $submission->payment_method != 'stripe') {
            return new \WP_Error('method_mismatch', 'Failed to cancel this subscription');
        }
        ...
        if(!$lastTransaction) {
            return new \WP_Error('transaction_mismatch', 'Failed to cancel this subscription');
        }

        $vendorSubscriptionId = $subscription->vendor_subscription_id;

        if(!$vendorSubscriptionId) {
            return new \WP_Error('no_vendor_subscription_found', 'Failed to cancel this subscription');
        }
        ...
        PaymentHelper::recordSubscriptionCancelled($subscription, $response, [
            ...
            'component'        => 'General',
            'status'           => 'info',
            'title'            => 'Subscription has been cancelled by '.$scope,
